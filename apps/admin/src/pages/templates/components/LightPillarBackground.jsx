...
function LightPillarBackground(
    ...
    React.useEffect(() => {
        ...
        const pillarRotRad = (Number(pillarRotation) * Math.PI) / 180;
        ...
        const material = new THREE.ShaderMaterial({
            ...
            uniforms: {
                ...
                uTopColor: { value: toColorVector(topColor) },
                uBottomColor: { value: toColorVector(bottomColor) },
                uIntensity: { value: Number(intensity) || 0 },
                uInteractive: { value: Boolean(interactive) },
                uGlowAmount: { value: Number(glowAmount) || 0 },
                uPillarWidth: { value: Number(pillarWidth) || 0 },
                uPillarHeight: { value: Number(pillarHeight) || 0 },
    ...
    React.useEffect(() => {
        ...
        materialRef.current.uniforms.uTopColor.value = toColorVector(topColor);
    }, [topColor]);

    React.useEffect(() => {
        if (!materialRef.current) return;
        materialRef.current.uniforms.uBottomColor.value = toColorVector(bottomColor);
    }, [bottomColor]);
    ...
    return <div ref={containerRef} className={`light-pillar-container ${className}`} style={{ mixBlendMode }} {...props} />;
}

const ForwardedLightPillarBackground = React.forwardRef(LightPillarBackground);
ForwardedLightPillarBackground.displayName = 'LightPillarBackground';

export default ForwardedLightPillarBackground;
